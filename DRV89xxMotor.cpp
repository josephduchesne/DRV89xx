...
DRV89xxMotor::DRV89xxMotor(byte hb1, byte hb2, byte pwm_channel) : _pwm_channel(pwm_channel) {
...
void DRV89xxMotor::setBridgeHSPWM(byte *settings, DRV89xxHalfBridge &bridge) {
  ...
  bitWrite(settings[bridge.pwm_map_register], bridge.bitshift_2, _pwm_channel & 1);  // write the low bit
  bitWrite(settings[bridge.pwm_map_register], bridge.bitshift_2 + 1, (_pwm_channel & 2) >> 1); // write the high bit
}
